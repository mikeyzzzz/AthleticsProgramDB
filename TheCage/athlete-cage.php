...
// Handle form submission for adding an attempt
if (isset($_POST['addAttempt'])) {
    // Validate and sanitize the input data
    $selectedEventID = $_POST['selectedEventID'];
    $recordInput = mysqli_real_escape_string($conn, $_POST['recordInput']);

    // Insert the new attempt into the Attempt table
    $insertAttemptQuery = "INSERT INTO Attempt (AthleteID, EventID, RecordInput, Verified)
                           VALUES ('$athleteID', '$selectedEventID', '$recordInput', false)";
    
    if ($conn->query($insertAttemptQuery) === TRUE) {
        echo "New attempt added successfully";
    } else {
        echo "Error: " . $insertAttemptQuery . "<br>" . $conn->error;
    }
}
?>
...
<div class="table-section">
    ...
    <?php endif; ?>
        ...
        <!-- Add Attempt Form -->
        <div class="add-attempt-form">
            <h2>Add Attempt to Leaderboard</h2>
            <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                <input type="hidden" name="selectedEventID" value="<?php echo $selectedEventID; ?>">
                <label for="recordInput">Record Input:</label>
                <input type="text" name="recordInput" required>
                <button type="submit" name="addAttempt">Add Attempt</button>
            </form>
        </div>
    </div>
</body>
